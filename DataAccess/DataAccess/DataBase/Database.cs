using DataAccess.DbExpand;
//using XNEW.DataAccess.DbExpand;
using System;
...
namespace DataAccess
{
    ...
    public class Database : IDatabase, IDisposable
    {
        ...
        /// <param name="strSql">Sql语句</param>
        /// <returns></returns>
        public DataTable FindTableByProcName(string procName)
        {
        ...
        /// 查询对象、返回实体
        /// eg: FindEntity(1);
        ///     FindEntity(1, "ID", "Name");
        ///     FindEntity(1, new string[] { "ID", "Name" });
        /// </summary>
        /// <param name="propertyValue">主键值</param>
        /// <returns></returns>
        public T FindEntity<T>(object propertyValue,  string[] fields) where T : new()
        {
            string pkName = DatabaseCommon.GetKeyField<T>().ToString();//获取主键字段
            StringBuilder strSql = DatabaseCommon.SelectSql<T>();
            if (fields != null && fields.Any()) strSql.Replace("*", string.Join(",", fields));
            strSql.Append(" AND ").Append(pkName).Append("=").Append(DbHelper.DbParmChar + pkName);
            switch (DbHelper.DbType)
            {
                case DatabaseType.SqlServer:
        ...
        /// <returns></returns>
        public DataTable QueryToDataTable(string commandText, CommandType commandType, DbParameter[] parameters)
        {
            return DbHelper.QueryToDataTable(commandText, commandType, parameters);
        }
        #endregion
        /// <summary>
        /// 查询返回Datatable
        ...
        /// <returns></returns>
        public DataTable FindTablePageByProc(string procName, DbParameter[] param, string orderField, string orderType, int pageIndex, int pageSize, ref int count)
        {
            return SqlServerHelper.GetPageTableProc(procName, param, orderField, orderType, pageIndex, pageSize, ref count);
        }
